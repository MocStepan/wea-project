...
class BookImportServiceTests : FeatureSpec({
  ...
  feature("save imported booking") {
    scenario("success") {
      ...
      spec.bookImportService.saveImportedBooks(importDTOs)
...
private class BookImportServiceSpecWrapper(
  val objectMapper: ObjectMapper,
  val bookImportRepository: BookImportRepository
) {

  val bookImportService: BookImportService = BookImportService(
    objectMapper,
    bookImportRepository
  )
}

private fun getSpec() = BookImportServiceSpecWrapper(objectMapper, mockk())
